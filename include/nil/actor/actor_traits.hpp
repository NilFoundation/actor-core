...
namespace nil::actor {
    ...
    struct default_actor_traits<T, true> {
        ...
        static constexpr bool is_blocking = std::is_base_of<blocking_actor_base, T>::value;
        ...
        static constexpr bool is_incomplete =
            (!is_dynamically_typed && !is_statically_typed) || (!is_blocking && !is_non_blocking);

        static_assert(!is_dynamically_typed || !is_statically_typed,
                      "an actor cannot be both statically and dynamically typed");

        static_assert(!is_blocking || !is_non_blocking, "an actor cannot be both blocking and non-blocking");
    };
