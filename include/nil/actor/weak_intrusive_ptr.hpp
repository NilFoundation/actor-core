...
namespace nil::actor {
    ...
    public:
        ...
        weak_intrusive_ptr(weak_intrusive_ptr &&other) noexcept : ptr_(other.detach()) {
            // nop
        }
        ...
        template<class Y>
        weak_intrusive_ptr(weak_intrusive_ptr<Y> other) noexcept : ptr_(other.detach()) {
    ...
    template<typename T>
    struct has_weak_ptr_semantics<weak_intrusive_ptr<T>> {
