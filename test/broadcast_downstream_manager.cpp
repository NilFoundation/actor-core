...
namespace {
    ...
    public:
        ...
        entity(actor_config &cfg, const char *cstr) : super(cfg), cstr_name(cstr), mgr(this), next_slot(1) {
            // nop
        }
    ...
    struct receive_checker {
        ...
        receive_checker(receive_checker &&x) :
            f(x.f), xs(std::move(x.xs)), moved_from(false), check_not_empty(x.check_not_empty) {
            x.moved_from = true;
        }
